...
var dbConfig = {
  ...
  db  : process.env.RDB_DB || 'psa_internal',
  tables: {
    'messages': 'id',
    'users': 'email'
  }
 ...
 *
 * - create the `RDB_DB` database (defaults to `psa_internal`)
 * - create tables `messages`, `users` in this database
 */
 ...
 */
module.exports.findUserByEmailOld = function (mail, callback) {
...
/**
 * Find a user by email (this is the primary key) Retrieving
 * a document by its primary key can be done using the
...
module.exports.findUserByEmail = function (email, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('users').get(email).run(connection, function(err, result) {
      if(err) {
...
module.exports.findUsers = function (max_results, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('users').limit(max_results).run(connection, function(err, results) {
      if(err) {
        logerror("[ERROR][%s][findUsers] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err, []);
      }
      else {
 ...
 */
var findMessages = function (max_results, callback) {
  onConnect(function (err, connection) {
...
var findMessagesWithUsername = function (max_results, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('messages').eqJoin('from', r.db('psa_internal').table('users')).zip().withFields('id', 'from', 'name' , 'message', 'timestamp').orderBy(r.desc('timestamp')).limit(max_results).run(connection, function(err, cursor) {
      if(err) {
        logerror("[ERROR][%s][findMessages] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err, []);
        connection.close();
...
module.exports.findMessagesWithUsername = findMessagesWithUsername;
/**
 * To find the last `max_results` messages ordered by `timestamp` they not have the old ids 'old_ids'
 *
 * @param {Number} max_results
 *    Maximum number of results to be retrieved from the db
 *
 * @param {Array} an array of ids
 *    The IDs which you do not want to have
 *
 * @param {Function} callback
 *    callback invoked after collecting all the results
 *
 * @returns {Array} an array of messages
 */
module.exports.findNewMessages = function (max_results, old_ids, callback) {
  if(old_ids && old_ids.length > 0) {
    onConnect(function (err, connection) {
      r.db(dbConfig['db']).table('messages').filter(r.expr(old_ids).contains(r.row('id')).not()).orderBy(r.desc('timestamp')).limit(max_results).run(connection, function(err, cursor) {
        if(err) {
          logerror("[ERROR][%s][findMessages] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
          callback(err, []);
          connection.close();
    ...
    findMessages(max_results, callback);
  }
};
